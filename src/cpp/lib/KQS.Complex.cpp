...
DeinterleaveAoSLComplex(std::span<const LComplex> arr) {
    // Ensure LComplex is standard layout and size is 16 bytes
    ...
    // [BENCHMARK] _DeinterleaveAoSLComplex
...
_DeinterleaveAoSLComplex<ExecutionPolicy::Parallel>(std::span<const LComplex> arr, std::span<double> res, std::span<double> ims) {
    ...
    if (rem > 0) {
        ...
        _DeinterleaveAoSLComplex<ExecutionPolicy::Sequential>(
            arr.subspan(offset, rem),
            res.subspan(offset, rem),
            ims.subspan(offset, rem)
        );
    }
}
...
inline
void
_CalculateProbabilities<ExecutionPolicy::Sequential>(typename DeviceContainer<ExecutionPolicy::Sequential, double>::ref_const_type res, typename DeviceContainer<ExecutionPolicy::Sequential, double>::ref_const_type ims, std::span<double> probs) {
    // [Sequential]
...
inline
void
_CalculateProbabilities<ExecutionPolicy::Parallel>(typename DeviceContainer<ExecutionPolicy::Parallel, double>::ref_const_type res, typename DeviceContainer<ExecutionPolicy::Parallel, double>::ref_const_type ims, std::span<double> probs) {
    // 1 Block = 4 Complex = 8 doubles = 2x 256-bit AVX2 registers
    // [Re0, Re1, Re2, Re3]
    // [Im0, Im1, Im2, Im3]
    ...
    std::for_each(std::execution::par, idxes.begin(), idxes.end(),
        [&] (size_t i) {
            ...
            // Streaming store to avoid cache pollution
    ...
    if (rem > 0) {
        ...
        _CalculateProbabilities<ExecutionPolicy::Sequential>(
            res.subspan(offset, rem),
            ims.subspan(offset, rem),
